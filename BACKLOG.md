...
| 19 | ~~Pre-Phase 4: Orchestrator refactor sprint~~ — **Done.** orchestrator.py (1030→228), theory_agent.py (929→215), sound_engineering_agent.py (394→232). __init__.py facade in place. | | | |
| 20 | **`production_agent.py` at 500 lines** — split into `production_agent.py` (class) and `production_local.py` (local lookup logic) in a future cleanup sprint. | Not blocking Phase 4. Production Agent is stable and tested. | Future cleanup | **Warning** — over 200-line limit |
| 21 | **`teaching_agent.py` at 444 lines** — split into `teaching_agent.py` (class) and `teaching_local.py` (local templates) in a future cleanup sprint. | Not blocking Phase 4. Teaching Agent is stable and tested. | Future cleanup | **Warning** — over 200-line limit |
| 22 | **`orchestrator_pipeline.py` at 393 lines** — review post-Phase 4 MCP. The legacy API lookup (`execute_local_lookup_legacy`, ~80 lines) may be refactored or removed once MCP is the primary integration path. | Legacy code from the `process()` API-mode path. May become dead code after MCP. | Post-Phase 4 | **Warning** — review after MCP integration |
...
| Backend scenario deep-research fan-out | Multi-agent audit of the local content engine: prompt-vocabulary → behavior grid (moods/genres/styles like "blues", "jazzy", "dusty"), per-instrument reference tables, expanded fixtures | After the /rubato/ demo clip ships | Medium — 2026-09-02 shoot surfaced repeated vocabulary gaps; Shannon approved running it after the clip is on the portfolio |
| Scenario research fan-out: EXECUTED 2026-09-02 | Full report at SCENARIO-RESEARCH.md — 5 lanes, theory-verified; Tier 1 = 9 small fixes where the app lies or goes blank (SE empty payload, dead lo-fi lookup, Massive Attack default, silent false Ableton success, setDone rewind, 0.0.0.0 bind); references lane produced nothing usable, needs a rerun if per-instrument reference tables are wanted | Next Rubato work session | High — supersedes the "deep-research fan-out" row above |
